...
module.exports.deleteProduct = async (req, res) => {
  sequelize.sync().then( async  () =>{    
  const product = await Product.destroy({ where :{ ShopId: req.body.ShopId, id: req.body.id } });
  if (!product) {
    return res.status(409).send({
...
module.exports.updateProduct = async (req, res) => {
  ...
  const product = await Product.findOne({ where :{ ShopId: req.body.ShopId, id: req.body.id } });
  if (!product) {
    return res.status(409).send({
...
module.exports.changeProductStatus = async (req, res) => {
 ...
 const condition ={ where :{ ShopId: req.body.ShopId, id: req.body.id } }
  ...
  const product = await Product.update({ product_available: req.body.product_available }, condition );
  if (!product) {
    return res.status(409).send({
      message: `No product found `,
      code:409,
